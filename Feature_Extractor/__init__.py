...
class main(): 
    ...
    def whois(self):
        ...
        tlds=lambda x: 1 if x.rsplit('.')[1] in str(tld) else 2 if x.rsplit('.')[1] in str(tld2) else 3 if x.rsplit('.')[1] in str(tld3) else 4
       ...
       #     "forms": (str(domain.html.forms[i].action) for i in range(0, len(domain.html.forms)))}})
